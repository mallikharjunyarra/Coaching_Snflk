...
create schema if not exists mydb.stge_tbls;
...
CREATE OR REPLACE TABLE mydb.stge_tbls.STG_BOOKS(xml_data variant);

// Load xml file to variant table
copy into mydb.stge_tbls.STG_BOOKS
from @mydb.external_stages.aws_s3_xml
files=('books_20230304.xml')
force=TRUE;

// Query stage table
select * from mydb.stge_tbls.STG_BOOKS;
...
select xml_data:"@" from mydb.stge_tbls.STG_BOOKS;

// To get root element value
select xml_data:"$" from mydb.stge_tbls.STG_BOOKS;

// Get the content using xmlget function, index position
select xmlget(xml_data,'book',0):"$" from mydb.stge_tbls.STG_BOOKS;
select xmlget(xml_data,'book',1):"$" from mydb.stge_tbls.STG_BOOKS;

// Fetch actual data from file
SELECT 
XMLGET(bk.value, 'id' ):"$" as "book_id",
XMLGET(bk.value, 'author' ):"$" as "author"
FROM mydb.stge_tbls.STG_BOOKS,
